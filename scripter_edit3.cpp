...
Scripter_edit3::Scripter_edit3(QWidget *parent) :
    QWidget(parent)
{
    margin = 20;
    ...
    connect(this, &Scripter_edit3::updatevalue_lineEdit, value_lineEdit, &QLineEdit::setText);
...
void Scripter_edit3::paintEvent(QPaintEvent *event)
{
    ...
    QPointF topLeft0(margin+value_edge,margin);
    QRectF rectangle0(topLeft, QSizeF(this->width()-margin*2, this->height()-margin*2));
    painter.drawRect(rectangle);
    for (int i = 0; i < values.count(); ++i){
        int index = i;
        int value = values[i];
...
void Scripter_edit3::mousePressEvent(QMouseEvent *event)
{
...
void Scripter_edit3::mouseMoveEvent(QMouseEvent *event)
{
    if (rebuild){
...
void Scripter_edit3::mouseReleaseEvent(QMouseEvent *event)
{
    press_point = move_point = event->position();
...
void Scripter_edit3::keyPressEvent(QKeyEvent *event)
{
...
void Scripter_edit3::keyReleaseEvent(QKeyEvent *event)
{
    key_shift = false;
    key_control = false;
    key_alt = false;
...
void Scripter_edit3::wheelEvent(QWheelEvent *event)
{
